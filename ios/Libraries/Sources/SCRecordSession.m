...
- (void)appendSegmentsToComposition:(AVMutableComposition *)composition audioMix:(AVMutableAudioMix *)audioMix withTimeModifiers:(NSArray *)timeModifiers {
  [self dispatchSyncOnSessionQueue:^{
    AVMutableCompositionTrack *audioTrack = nil;
    AVMutableCompositionTrack *videoTrack = nil;
    ...
    for (SCRecordSessionSegment *recordSegment in _segments) {
      ...
      CMTimeRange timeRange = CMTimeRangeMake(currentTime, composition.duration);

      CMTime modifiedTime = CMTimeMake(composition.duration.value * [(NSNumber *)[timeModifiers objectAtIndex:currentSegment] floatValue], composition.duration.timescale);
      [composition scaleTimeRange:timeRange toDuration:modifiedTime];
      currentTime = composition.duration;
...
- (AVAsset *)assetRepresentingSegmentsWithTimeModifiers:(NSArray *)timeModifiers {
  ...
  [self dispatchSyncOnSessionQueue:^{
    AVMutableComposition *composition = [AVMutableComposition composition];
    [self appendSegmentsToComposition:composition audioMix:nil withTimeModifiers:timeModifiers];
    asset = composition;
  }];
